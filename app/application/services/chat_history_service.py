...
class ChatHistoryService:
    ...
    async def get_user_statistics(self, user_id: str) -> dict:
        ...
        active_sessions = len([s for s in sessions if not s.is_archived and not s.is_deleted])
        ...
        for session in sessions:
            ...
            if session.title == "New Conversation" and session.message_count > 0:
                ...
                if messages:
                    ...
                    preview = "\n".join([f"{m.role.value}: {m.content[:100]}" for m in reversed(messages)])
                    
                    # Generate title
                    prompt = f"Generate a short title (max 10 words) for this conversation:\n\n{preview}\n\nTitle:"
                    title = await self.llm_service.generate(prompt, temperature=0.3, max_tokens=20)
