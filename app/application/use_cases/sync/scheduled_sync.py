...
class ScheduledSyncUseCase:
    ...
    async def schedule_sync(self, input_data: ScheduledSyncInput) -> ScheduledSyncOutput:
        """
        ...
        data_source.sync_schedule = {
            ...
            "interval_hours": input_data.interval_hours if input_data.schedule_type == "interval" else None,
        ...
        logger.info(f"Scheduled sync for {input_data.data_source_id}, next run: {next_run}")
        
        return ScheduledSyncOutput(
            success=True,
            job_id=job_id,
            next_run_time=next_run,
            message=f"Sync scheduled successfully"
        )
...
class SyncExecutor:
    ...
    async def sync_data_source(self, data_source_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result["status"] = "partial" if result["documents_processed"] > 0 else "failed"
            result["errors"].append(str(e))
    ...
    async def _fetch_file_content(self, data_source: DataSource) -> List[Dict[str, Any]]:
