...
ARTIFACT_KEYWORDS = [
    ...
    "biểu mẫu", "file mẫu", "template",
    # Download requests
    "tải file", "download", "tải về", "xin file", "cho file", "lấy file",
    "tải xuống", "file", "tài liệu",
    # Document requests  
    "quy trình", "hướng dẫn làm", "mẫu giấy", "giấy tờ",
    # Specific forms
    "đơn nghỉ học", "đơn xin vắng", "đơn đăng ký", "giấy xác nhận",
]

# Keywords for form filling intent
FILLABLE_KEYWORDS = [
    "điền", "điền sẵn", "điền giúp", "điền thông tin", "fill",
    "tự động điền", "điền form",
]
    ...
    def _detect_intent(self, query: str) -> ResponseIntent:
        ...
        for keyword in ARTIFACT_KEYWORDS:
            if keyword in query_lower:
                ...
                if any(k in query_lower for k in ["mẫu đơn", "form", "đơn xin", "biểu mẫu"]):
                    return ResponseIntent.FORM_REQUEST
                return ResponseIntent.FILE_REQUEST
        
        # Check for procedure/guide request
        if any(k in query_lower for k in ["quy trình", "cách làm", "hướng dẫn", "thủ tục"]):
            return ResponseIntent.PROCEDURE_GUIDE
        
        # Check for contact info
        if any(k in query_lower for k in ["liên hệ", "số điện thoại", "email", "địa chỉ"]):
