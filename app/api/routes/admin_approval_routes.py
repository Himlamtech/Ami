...
def _to_response(pending) -> PendingUpdateResponse:
    ...
    content_preview = pending.content[:500] + "..." if len(pending.content) > 500 else pending.content
    ...
    return PendingUpdateResponse(
        ...
        category=pending.category.value if hasattr(pending.category, 'value') else pending.category,
        detection_type=pending.detection_type.value if hasattr(pending.detection_type, 'value') else pending.detection_type,
