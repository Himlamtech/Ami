...
from app.application.use_cases.data_source.create_data_source import CreateDataSourceInput
from app.application.use_cases.data_source.list_data_sources import ListDataSourcesInput
from app.application.use_cases.data_source.update_data_source import UpdateDataSourceInput
from app.application.use_cases.data_source.delete_data_source import DeleteDataSourceInput
...
def _to_response(source: DataSource) -> DataSourceResponse:
    ...
    if source.auth and source.auth.auth_type != "none":
        auth_config = AuthConfigResponse(
            ...
            has_credentials=bool(source.auth.username or source.auth.token or source.auth.cookies),
        )
    ...
    return DataSourceResponse(
        ...
        category=source.category.value if isinstance(source.category, DataCategory) else source.category,
        data_type=source.data_type.value if hasattr(source.data_type, 'value') else source.data_type,
        schedule_cron=source.schedule_cron,
        crawl_config=crawl_config,
        auth_config=auth_config,
        status=source.status.value if isinstance(source.status, SourceStatus) else source.status,
...
async def update_data_source(
    ...
    input_data = UpdateDataSourceInput(
        ...
        rate_limit=int(request.crawl_rate_limit_delay) if request.crawl_rate_limit_delay else None,
    )
...
async def test_data_source(
    ...
    input_data = TestDataSourceInput(
        url=request.url,
        source_type=SourceType(request.source_type) if request.source_type else SourceType.WEB_CRAWL,
