...
class MongoDBPendingUpdateRepository(IPendingUpdateRepository):
    ...
    async def check_duplicate(self, content_hash: str) -> bool:
        ...
        doc = await self.collection.find_one({
            ...
            "status": {"$in": [PendingStatus.PENDING.value, PendingStatus.APPROVED.value]}
        })
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_detection_type": {item["_id"]: item["count"] for item in data.get("by_detection_type", [])},
            "by_source": {item["_id"]: item["count"] for item in data.get("by_source", [])},
        }
