...
export default function ChatInput({
    ...
    const handleSend = () => {
        if (message.trim() || attachments.length > 0) {
            onSend(message.trim(), attachments, currentMode)
            setMessage('')
            setAttachments([])
        }
    }
                ...
                {attachments.length > 0 && (
                    <div className="flex gap-2 mb-3 flex-wrap">
                        {attachments.map((attachment, index) => (
                            <div
                                key={index}
                                className="relative flex items-center gap-2 px-3 py-2 bg-[var(--surface2)] rounded-xl shadow-sm group"
                            >
                                ...
                                <button
                                    ...
                                    className="p-0.5 hover:bg-[var(--surface)] rounded opacity-0 group-hover:opacity-100 transition-opacity"
                                >
