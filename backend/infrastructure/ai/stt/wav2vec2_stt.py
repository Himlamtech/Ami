...
class Wav2Vec2STTService(ISTTService):
    ...
    def __init__(
        ...
        Args:
            config: STT configuration. If None, uses global stt_config.
        ...
        logger.info(
            f"Wav2Vec2STTService initialized (model: {self.model_name}, device: {self.device}, "
    ...
    def _load_model_sync(self):
        ...
        try:
            # Download model_handling.py from HuggingFace Hub
            model_path = hf_hub_download(
                repo_id=self.model_name, filename="model_handling.py"
            )
    ...
    def _transcribe_sync(self, file_path: str, use_lm: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            input_data = self.processor.feature_extractor(
                audio, sampling_rate=self.TARGET_SAMPLE_RATE, return_tensors="pt"
            )
            ...
            if use_lm:
                text_with_lm = self.processor.decode(
                    output.logits.cpu().detach().numpy()[0], beam_width=100
                ).text
            else:
        ...
        try:
            ...
            try:
                audio_array, sample_rate = librosa.load(
                    file_path, sr=self.TARGET_SAMPLE_RATE, mono=True
                )
...
if __name__ == "__main__":
    ...
    async def main():
        stt_service = Wav2Vec2STTService(lazy_load=False)
