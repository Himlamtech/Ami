...
class MongoDBClient:
    ...
    def __init__(self):
        ...
        Configuration is loaded from environment variables via mongodb_config.
        """
        self._config = mongodb_config
        self.database_name = self._config.database

        self.client: Optional[AsyncIOMotorClient] = None
        self.db: Optional[AsyncIOMotorDatabase] = None

        logger.info(f"Initialized MongoDBClient (db={self.database_name})")
    ...
    async def list_documents(
        ...
        cursor = (
            self.db.documents.find(query).skip(skip).limit(limit).sort("created_at", -1)
        )
    ...
    async def create_chat_session(self, session_data: Dict[str, Any]) -> str:
        ...
        logger.info(
            f"Created chat session: {session_data.get('title')} for user {session_data.get('user_id')}"
        )
        return str(result.inserted_id)
    ...
    async def update_chat_session(
        ...
        result = await self.db.chat_sessions.update_one(
            {"_id": ObjectId(session_id), "is_deleted": False}, {"$set": update_data}
        )
        return result.modified_count > 0
    ...
    async def list_chat_messages(
        ...
        """
        cursor = (
            self.db.chat_messages.find({"session_id": session_id, "is_deleted": False})
            .skip(skip)
            .limit(limit)
    ...
    async def update_chat_message(
        ...
        result = await self.db.chat_messages.update_one(
            {"_id": ObjectId(message_id), "is_deleted": False}, {"$set": update_data}
        )
        return result.modified_count > 0
    ...
    async def delete_session_messages(self, session_id: str) -> int:
        ...
        result = await self.db.chat_messages.update_many(
            {"session_id": session_id}, {"$set": {"is_deleted": True}}
        )
