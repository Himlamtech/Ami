...
class FeedbackService:
    ...
    async def _update_stats(self, feedback: MessageFeedback) -> None:
        ...
        update_ops: Dict[str, Any] = {"$inc": {"total_feedbacks": 1}}
        ...
        await self.stats_collection.update_one(
            {"_id": "global"}, update_ops, upsert=True
        )
    ...
    def analyze_feedback_for_prompt(
        ...
        if FeedbackCategory.COMPLETENESS in [
            c for fb in feedbacks for c in fb.categories
        ]:
