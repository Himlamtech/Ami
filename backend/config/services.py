...
class ServiceRegistry:
    ...
    def _ensure_initialized(cls):
        ...
        if not cls._initialized or cls._db is None:
            raise RuntimeError(
                "ServiceRegistry not initialized. Call ServiceRegistry.initialize(db) first."
            )
    ...
    @classmethod
    def get_llm(
        cls, provider: str = "openai", mode: LLMMode = LLMMode.QA
    ) -> ILLMService:
    ...
    def get_web_crawler(cls):
        ...
        if cls._web_crawler is None:
            ...
            cls._web_crawler = FirecrawlCrawler()
    ...
    def get_document_resolver(cls):
        ...
        if cls._document_resolver is None:
            ...
            llm = cls.get_llm()
            cls._document_resolver = DocumentResolver(
                embedding_service=embedding,
                vector_store=vector_store,
                llm_service=llm,
            )
        return cls._document_resolver
    ...
    def get_document_ingest_service(cls):
        ...
        if cls._document_ingest_service is None:
            ...
            pending_repo = cls.get_pending_update_repository()
            document_repo = cls.get_document_repository()
            resolver = cls.get_document_resolver()
            cls._document_ingest_service = DocumentIngestService(
                pending_repo=pending_repo,
                document_repo=document_repo,
                resolver=resolver,
            )
        return cls._document_ingest_service
    ...
    @classmethod
    def get_monitor_target_repository(cls):
        ...
        if cls._monitor_target_repo is None:
            ...
            cls._monitor_target_repo = MongoDBMonitorTargetRepository(cls._db)
        return cls._monitor_target_repo
