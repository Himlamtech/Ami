...
async def create_collection(
    ...
    result = use_case.execute(
        CreateCollectionInput(name=request.name, vector_size=request.vector_size)
    )
...
async def get_collection_info(
    ...
    return CollectionInfoResponse(
        name=result.name, exists=result.exists, info=result.info
    )
...
async def scroll_documents(
    ...
    return ScrollResponse(
        documents=result.documents, next_offset=result.next_offset, total=result.total
    )
...
async def get_document(
    ...
    result = use_case.execute(
        GetDocumentInput(document_id=document_id, collection_name=collection_name)
    )
...
async def delete_documents_by_filter(
    ...
    result = use_case.execute(
        DeleteDocumentsByFilterInput(
            filter_conditions=request.filter_conditions, collection_name=collection_name
        )
    )
